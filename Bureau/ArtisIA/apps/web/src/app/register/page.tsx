...
export default function RegisterPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.error(err);
      setError("Erreur avec l'inscription Google.");
    }
  };
  ...
  return (
    <AuthLayout 
      ...
      subtitle="Créez votre compte en quelques secondes. Aucune carte bancaire requise."
    >
      <motion.div
      ...
      >
        <button 
          ...
          className="w-full flex items-center justify-center gap-3 bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 font-medium py-3 px-4 rounded-xl transition-all duration-200"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-blue-700 focus:ring-4 focus:ring-blue-500/20 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? (
